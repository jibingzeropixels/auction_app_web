...
const PlayerTradingCard: React.FC<PlayerTradingCardProps> = ({ player }) => {
  ...
  const adjustColor = (color: string, amount: number) => {
    return '#' + color.replace(/^#/, '').replace(/../g, color => {
      const colorValue = Math.min(255, Math.max(0, parseInt(color, 16) + amount)).toString(16);
  ...
  return (
    ...
    >
      ...
      >
        <Typography variant="h5" fontWeight="bold" sx={{ textShadow: '1px 1px 2px rgba(0,0,0,0.5)', position: 'relative', zIndex: 2 }}>
          {fullName}
        </Typography>
        ...
        {isIcon && (
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', mt: 0.5 }}>
            ...
            <Typography variant="subtitle2" sx={{ textShadow: '1px 1px 2px rgba(0,0,0,0.5)', position: 'relative', zIndex: 2 }}>
              Icon Player
            </Typography>
          </Box>
        )}
      ...
      <Box sx={{ 
        ...
        <PersonIcon sx={{ position: 'absolute', top: 10, right: 10, opacity: 0.1, fontSize: 30 }} />
        <PersonIcon sx={{ position: 'absolute', bottom: 10, left: 10, opacity: 0.1, fontSize: 30 }} />
      </Box>
